...
class LangGraphUtils:
    ...
    def to_base_message(langchain_message: LangchainBaseMessage) -> BaseMessage:
        if isinstance(langchain_message, AIMessage):
            return BaseMessage(
                message_type=BaseMessageType.AiMessage,
                content=str(langchain_message.content),
            )
        elif isinstance(langchain_message, HumanMessage):
            return BaseMessage(
                message_type=BaseMessageType.HumanMessage,
                content=str(langchain_message.content),
            )
        else:
            raise NotImplementedError(
                f"Unexpected message type: {type(langchain_message)}"
            )
